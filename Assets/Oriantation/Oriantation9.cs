...
public class Oriantation9 : MonoBehaviour
{
    // 要隱藏或顯示的 CanvasGroup
    ...
    void Update()
    {
        if (canvasGroup != null)
        {
            ...
            if (!string.IsNullOrEmpty(Oriantation.messageString) && Mes.messageString.Length > 2)
            {
                // 獲取第二個字符並進行比較
                char secondChar = Oriantation.messageString[0];
                if (secondChar == '9')
                {
                    isFadingIn = true;
                }
